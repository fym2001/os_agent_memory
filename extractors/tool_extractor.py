...
def _event_scenario(events: list[MemoryEvent]) -> Scene:
    ...
    return scenario if all(event.scenario == scenario for event in events) else Scene.TOOL
...
class ToolExtractor:
    ...
    def extract_tool_pattern(events: list[MemoryEvent]) -> list[MemoryCandidate]:
        ...
        for (user_id, tool_name), invocations in sorted(by_user_tool.items()):
            ...
            scenario = scenarios[0] if all(item == scenarios[0] for item in scenarios) else Scene.TOOL
