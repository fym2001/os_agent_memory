...
def test_core_models_and_storage_round_trip(tmp_path: Path) -> None:
    event = MemoryEvent(
        ...
        raw_event={"nested": {"value": 1}},
    )

    event_dict = event.to_dict()
    assert event_dict["event_type"] == "conversation"
    assert event_dict["scenario"] == "chat"
    ...
    candidate = MemoryCandidate(
        ...
        created_at="2026-06-23T01:00:00+00:00",
    )
    candidate.with_context(user_id="user-final", scenario="chat", source_event_id="evt-c")

    candidate_dict = candidate.to_dict()
    assert candidate_dict["memory_type"] == "knowledge"
    assert candidate_dict["user_id"] == "user-final"
    assert "evt-c" in candidate_dict["source_events"]
    assert candidate_dict["metadata"]["tuple"] == [1, 2]

    record = MemoryRecord.from_candidate(candidate, updated_at="2026-06-23T02:00:00+00:00")
    record_dict = record.to_dict()
    assert record_dict["memory_type"] == "knowledge"
    assert record_dict["updated_at"].startswith("2026-06-23T02:00:00")
    assert record_dict["metadata"]["flags"] == [True, False]

    db_path = tmp_path / "memories.sqlite3"
    persisted = save_memory(db_path, candidate)
    with sqlite3.connect(db_path) as connection:
        row = connection.execute(
            "SELECT user_id, memory_type, key, content FROM memories"
        ).fetchone()
    assert row == ("user-final", "knowledge", "knowledge.example", "候选内容")
...
def test_preference_internal_helper_branches() -> None:
    ...
    deduped = preference_mod._dedupe_candidates(
        [
            MemoryCandidate(user_id="u", memory_type=MemoryType.PREFERENCE, key="preference.test.x", content="a"),
            MemoryCandidate(user_id="u", memory_type=MemoryType.PREFERENCE, key="preference.test.x", content="b"),
        ]
    )
...
def test_preference_implicit_internal_branches() -> None:
    ...
    assert any(candidate.scenario is Scene.CHAT for candidate in implicit)
...
def test_knowledge_template_variants_and_dedup() -> None:
    ...
    assert any(candidate.scenario is Scene.CHAT for candidate in candidates)
    assert any(candidate.scenario is Scene.CODE for candidate in candidates)

    deduped = knowledge_mod._dedupe_candidates(
        [
            MemoryCandidate(user_id="u", memory_type=MemoryType.TEMPLATE, key="template.generic.x", content="a"),
            MemoryCandidate(user_id="u", memory_type=MemoryType.TEMPLATE, key="template.generic.x", content="b"),
        ]
    )
    assert len(deduped) == 1
